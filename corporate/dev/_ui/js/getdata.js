...
function updateClient() { // clientId, values, optional sendemail=false, callback){
    if (arguments.length!=3 && arguments.length!=4) {
        throw "Invalid arguments.  Must send: clientId, values, callback or clientId, values, sendemail, callback.";
        return;
    }
...
function downloadAccountingReport(startdate, enddate, organizationId, tags, callback) {
    if (tags=="") tags = "all";
    window.open(SERVER_URI +'/report/' + startdate + "/" + enddate + "/" + organizationId + "/" + tags + "/csv");
