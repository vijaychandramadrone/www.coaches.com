...
function custom_get($obj) {
    if ($obj->table=="progress_report") { //start_date, end_date, orgid or client id,  tags (optional)
        ...
        if ($obj->userType==2 || $obj->userType==1 || $obj->userType==3 ) { //progress report
            $sql = "SELECT CONCAT(cl.first_name, ' ', cl.last_name) as client, CONCAT(co.first_name, ' ', co.last_name) as coach, s.session_datetime, s.duration, (CASE status_code WHEN 1 THEN (CASE progress_notes_approved WHEN 1 THEN progress_notes ELSE 'not yet approved' END) WHEN 2 THEN 'NO SHOW' WHEN 3 THEN 'CANCELED'  END) as notes FROM session_view s INNER JOIN clients_users cl ON cl.id=s.client_id INNER JOIN coachs_users co ON co.id=s.coach_id WHERE s.status_code>0 AND s.status_code < 4 AND s.client_id=" . $obj->uid[2];
            //This returns UTC time.  We could update to parse through rows and update time to accounting time...
            $obj->respondWithCSV($sql,"Client,Coach,Date,Duration (minutes),Notes");
        } else {
            $obj->unauthorized();
        }
        return true;
        ...
        if ($obj->userType==5 || $obj->userType==1 ) { //accounting report
            ...
            $csvSql = "SELECT CONCAT(client_first_name,' ', client_last_name) as client_name, CONCAT(coach_first_name, ' ', coach_last_name) as coach_name, session_start_datetime, (CASE status_code WHEN 1 THEN 'COMPLETE' WHEN 2 THEN 'NO SHOW' WHEN 3 THEN 'CANCELED'  END) as status, bill_rate, pay_rate, net_rate, tags ";
            ...
            $fromSql = " FROM accounting_session_view WHERE organization_id=$orgId AND session_datetime>='$start' AND session_datetime<='$end' ";
            ...
            if ($returnCSV) {
                //returns UTC times
                $obj->respondWithCSV($csvSql, "Client,Coach,SessionDate,Status,Fee,Expense,Net,Tags");
                return true;
            }
...
//            $sql = "SELECT d.* FROM documents d INNER JOIN clients ON d.client_id=clients.id INNER JOIN organizations_users ou ON clients.organization_id=ou.id WHERE ou.user_id=" . $obj->userId . " AND d.client_id=" . $obj->uid[0];
        ...
        $ret = array();
        $sql = "SELECT o.budget, SUM(sessions.bill_rate) as total_bill_rate FROM sessions, clients cl, organizations o WHERE o.id=cl.organization_id AND cl.id=sessions.client_id  AND cl.organization_id=$orgId GROUP BY cl.organization_id";
        $resource = $obj->db->executeStmt($sql);
