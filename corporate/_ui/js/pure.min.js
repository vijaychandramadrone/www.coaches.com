...
if(i===""){l=c[c.length-1];if((e=l.search(/[^\s]+=\"?$/))>-1){c[c.length-1]=l.substring(0,e);k=k.substr(1)}}c[c.length]=i;c[c.length]=k}return c.join("")}catch(m){if(!isIE && console&&console.log)console.log(m.stack?m.stack:m.message+" ("+m.type+", "+m.arguments.join("-")+"). Use Firefox or Chromium/Chrome to get a full stack of the error. ");return""}}}function T(a){var b=a.match(/^(\w+)\s*<-\s*(\S+)?$/);b===null&&s('bad loop spec: "'+a+'"');b[1]==="item"&&s('"item<-..." is a reserved word for the current running iteration.\n\nPlease choose another name for your loop.');
